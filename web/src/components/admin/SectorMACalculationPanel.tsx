 ...
 * 允许管理员计算或更新板块移动平均线数据。
 */
export default function SectorMACalculationPanel() {
  useRequireAdmin();
  ...
  const startCalculation = async () => {
    try {
      ...
      if (calculationMode === 'by-date') {
        taskType = 'backfill_sector_ma_by_date';
        params = {
          target_date: selectedDate,
          overwrite
        };
      } else if (calculationMode === 'by-range') {
        taskType = 'calculate_sector_ma';
        params = {
        ...
        // full-history
        taskType = 'calculate_sector_ma_full_history';
        params = {
          periods,
          overwrite,
          sector_id: targetType === 'single' ? String(sectorId) : undefined
      ...
      setTasks(prev => ({
        ...
        [taskId]: {
          ...
          current: 0,
          total: 0,
          message: `已创建${calculationMode === 'by-date' ? '按日期' : calculationMode === 'by-range' ? '按时间段' : '完整历史'}板块均线计算任务`
        }
      }));
  ...
  const togglePeriod = (period: number) => {
    if (periods.includes(period)) {
      ...
      setPeriods([...periods, period].sort((a, b) => a - b));
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-900 mb-2">板块均线计算</h2>
        <p className="text-gray-600">
          计算或更新板块移动平均线数据，支持多种周期和日期范围。
        </p>
      </div>
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              ...
              <div className="mt-1 text-sm text-green-700">
                <p>此模式将自动从每个板块的最早数据日期开始，逐步计算到最新数据的所有均线。</p>
      ...
      {/* 目标选择 */}
      {(calculationMode === 'by-range' || calculationMode === 'full-history') && (
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
      ...
      <div className={`bg-white rounded-lg shadow-sm border p-6 ${
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <label htmlFor="overwrite-ma" className="flex items-center gap-2 text-sm font-medium text-gray-700 cursor-pointer">
              <AlertTriangle className={`w-4 h-4 ${overwrite ? 'text-orange-600' : 'text-gray-400'}`} />
              覆盖已有均线数据
            </label>
            <p className="mt-1 text-sm text-gray-600">
              {overwrite
                ? '将强制更新已有均线数据，用于修正异常计算结果。此操作不可撤销。'
                : '只添加缺失的均线数据，不修改已有数据。'}
            </p>
            {overwrite && (
              <div className="mt-3">
      ...
      <div className="flex flex-wrap gap-3">
        <button
          ...
          disabled={loading || (overwrite && !confirmOverwrite) ||
            ((calculationMode === 'by-range' || calculationMode === 'full-history') && targetType === 'single' && !sectorId)}

...
export const RankingItemComponent: React.FC<RankingItemProps> = React.memo(({ data, type }) => {
  ...
  return (
    <div
      className="flex items-center justify-between p-3 border-b border-border hover:bg-background cursor-pointer transition-colors"
      onClick={handleClick}
    >
      <div className="flex items-center gap-3 flex-1">
        {/* 排名编号 */}
        <span className="text-sm font-semibold text-faint w-6 text-center">
          {data.rank}
        </span>

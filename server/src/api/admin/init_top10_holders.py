...
@router.post("/top10-holders", response_model=ApiResponse[dict])
async def init_top10_holders(
    request: InitTop10HoldersRequest,
    ...
    创建 SYNC_TOP10_HOLDERS 异步任务，从 Tushare 拉取指定报告期的十大流通股东数据。
    """
    # 并发保护：检查是否有同类型 running 任务
    running = await session.execute(
        select(AsyncTask).where(
            and_(
                AsyncTask.task_type == TaskType.SYNC_TOP10_HOLDERS.value,
    ...
    if running.scalar_one_or_none():
        return ApiResponse(
            success=False,
            data=None,
            message="已有股票持仓同步任务正在运行，请等待当前任务完成",
        )
    ...
    task = await manager.create_task(
        task_type=TaskType.SYNC_TOP10_HOLDERS.value,
    ...
    return ApiResponse(
        ...
        message=f"股票持仓同步任务已创建（报告期: {request.period}）",
    )

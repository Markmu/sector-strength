"""融资融券查询 API 路由（第 17 期 plan-06）

端点：
- GET  /api/v1/margin/trend  — 全市场融资融券趋势（AC-5 30/90/250 裁剪、缺口 null）

复用声明（范式照搬 src/api/v1/market_metrics.py，第 16 期 plan-06）：
- 路由 + helper 范式：src/api/v1/index_monitor.py（_serialize_value / _dict_to_camel /
  {success,data} 包裹 / Depends(get_current_user)）
- 2 个模型：src/models/trading_calendar_day.py（TradingCalendarDay，16 期交付本期复用）、
  src/models/market_margin_daily.py（MarketMarginDaily，17 期 plan-01 交付）

契约（spec REQ-6 + plan-06 §3）：
- 路径：router prefix /margin + v1 主路由 prefix /v1 = /api/v1/margin/*
- query 参数名 ``range``（单词无 snake/camel 歧义），仅允许 30/90/250，非法值 422
- 响应：``{ success: bool, data: {...} }`` 包裹，data 内字段经 _dict_to_camel 转 camelCase，
  Decimal → float（不得输出字符串）、date → ISO 字符串
- 输出契约 = ``MarginTrendData``：
  ``{ latest: MarginPoint | null, points: MarginPoint[], range, hasMissingDates }``，
  point 七键 ``tradeDate/rzye/rqye/rzmre/rzche/rqmcl/rzrqye``（rqyl 不落库不输出）
- 单位口径：存储元、API 输出元原始值 float；÷1e8 转亿只在 plan-07 前端显示层

GET 读路径硬约束（spec REQ-6、spec D4 范式）：
- 零 Provider 调用：禁止实例化 TradingCalendar（缓存未命中会实时访问 Provider）、
  禁止调用 DataSourceFactory / Tushare —— 本文件不 import 任何 Provider 侧模块
- 缺失日六指标输出 null（不补 0 / 前值）；latest/hasMissingDates 判据取 rzye
  （16 期取 volume_shares 的同位裁定），latest 取最近"有结果"日（不伪装今天）
"""
...
# ============== Helper（复制自 index_monitor.py，与 market_metrics 同源惯例）==============
...
def _to_point(cal: date, metric: MarketMarginDaily | None) -> dict:
    """构造单个交易日点（snake_case，后续统一经 _dict_to_camel 转 camelCase）。

    缺失日（无 market_margin_daily 行）六项指标全 null —— 不补 0 / 前值（AC-5）。
    rqyl（融券余量，股）不落库不输出（spec D1 / plan-01 口径）。
    """
    if metric is None:
        return {
            "trade_date": cal,
...
async def get_trend(
    ...
    # 默认 30、非法值 422（16 期 market_metrics.py:110 实测教训，17 期照搬）。
    ...
    流程（spec REQ-6、照搬 market_metrics.py:129-208 结构）：
    ...
    3. 开市日轴 LEFT JOIN market_margin_daily（参数化 IN，升序）；
    4. points 逐日构造，缺结果日六项指标全 null（不补 0/前值）；
    5. latest = points 自尾向头第一个 rzye 有值点（展示最近成功结果日，不伪装今天）；
    6. hasMissingDates = 任一点 rzye 为 null。
    ...
    """
    # ``range`` 为 Python 内建名，端点内转 int 并改用 range_days，避免遮蔽（16 期风险备注照搬）
    ...
    try:
        ...
        if not dates:
            logger.warning(
                "margin trend: 交易日历未初始化（trading_calendar_days 无开市日），"
                "range=%d", range_days,
            )
            return {
                "success": False,
                "data": None,
                "message": "交易日历未初始化，请先执行融资融券同步",
            }

        # 3. 开市日轴 LEFT JOIN 两融汇总（参数化 IN，升序；缺失日 m 为 None）
        metric_res = await session.execute(
            select(TradingCalendarDay.cal_date, MarketMarginDaily)
            .outerjoin(
                MarketMarginDaily,
                MarketMarginDaily.trade_date == TradingCalendarDay.cal_date,
            )
            .where(
        ...
        # 4. points 逐日构造（缺失日六指标全 null，不补 0/前值）
        points = [_to_point(d, metric_map.get(d)) for d in dates]

        # 5. latest：自尾向头第一个 rzye 有值点；全空 → null
        latest = None
        for p in reversed(points):
            if p["rzye"] is not None:
                latest = p
                break

        # 6. hasMissingDates：任一点 rzye 为 null
        missing_count = sum(1 for p in points if p["rzye"] is None)
        has_missing_dates = missing_count > 0

        # 可观测性：range/points/missing_count/db_duration_ms（前缀 margin trend）；
        # 无 Provider 调用可记录（GET 路径天然零调用）
        db_duration_ms = (time.perf_counter() - db_start) * 1000
        logger.info(
            "margin trend: range=%d, points=%d, missing_count=%d, db_duration_ms=%.1f",
            range_days,
            len(points),
            missing_count,
            db_duration_ms,
        ...
        logger.exception("margin get_trend error, range=%s", range)
        raise

"""融资融券范围同步专用路由测试（第 17 期 plan-05）

覆盖 plan-05 §5 验收标准 AC-4（五项日期校验拒绝且不建任务）、AC-3 端点侧
（互斥拒绝）、require_admin 403、合法请求返回 task_id 且任务类型/params/
created_by 接线正确（task_type="sync_market_margin"）。

tests/api/admin/conftest.py 的 autouse fixture 将 get_current_user 覆盖为
admin，故鉴权默认通过；403 用例在本测试内再次覆盖为非 admin 用户。与 16 期
test_init_market_metrics.py 同款隔离：mock TradingCalendarRepository 与
TaskManager.create_exclusive_task，不触碰共享库（避免运行中的服务实例拾取）。
"""
...
class TestInitMarginRoute:
    """专用路由五项校验链 / 互斥 / 鉴权测试。"""

    def test_valid_request_creates_margin_task(self, client):
        """合法起止日：返回 task_id，且接线 task_type/params/created_by。"""
        repo = _repo_mock(
            open_count=2, trading_days=[date(2026, 8, 11), date(2026, 8, 12)]
        )
        admin_id = uuid.uuid4()

        def _fixed_admin_user():
            user = _admin_user()
            user.id = admin_id
            return user

        fastapi_app = _unwrap_fastapi(app)
        fastapi_app.dependency_overrides[get_current_user] = _fixed_admin_user
        try:
            fake_task = MagicMock()
            fake_task.task_id = "task_abcdef123456"
            with patch(
                "src.api.admin.init_margin.TradingCalendarRepository",
                return_value=repo,
            ), patch(
        ...
        assert "交易日 2 个" in data["message"]
        mock_create.assert_awaited_once()
        kwargs = mock_create.await_args.kwargs
        assert kwargs["task_type"] == "sync_market_margin"
    ...
    def test_end_after_today_rejected_no_task(self, client):
        """end>today：success=False 且不建任务（AC-4 主断言）。"""
    ...
    def test_span_over_10_years_rejected_no_task(self, client):
        """跨度>10 年：success=False 且不建任务（AC-4）。"""
        end = date.today()
    ...
    def test_mutex_rejected_when_existing_running(self, client):
        """互斥命中：create_exclusive_task 返回 None → success=False 提示（AC-3 端点侧）。"""
        repo = _repo_mock(
            open_count=2, trading_days=[date(2026, 8, 11), date(2026, 8, 12)]
        )
        with patch(
        ...
        data = resp.json()
        assert data["success"] is False
        assert "已有融资融券同步任务正在运行" in data["message"]

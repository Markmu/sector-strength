"""市场量价查询 API 测试 — plan-06（查询 API）

覆盖端点：GET /api/v1/market-metrics/trend（架构 §6.4.2、§7.3 + plan-06 §5）：
- AC-05：range=30/90/250 服务端裁剪；range=50 → 422
- AC-06：5 日轴 + 3 日数据 → 5 点、缺失日五项指标全 null、hasMissingDates=true、
  latest 为最后一个有值日（无 0/前值填充）
- 契约：camelCase 字段、Decimal → float、日期 ISO 字符串、{success,data} 包裹
- 空态：日历空表 → success=False + 未初始化 message；日历有开市日但指标全空
- 鉴权：未登录（无 token）→ 401；普通登录用户可读（AC-11 查询权限侧）
- 零 Provider：mock DataSourceFactory / TradingCalendar / TushareDataSource，断言
  GET 路径零调用（架构 §4.2 模块 4、ADR-6）
- 性能：250 日查询单次 < 500ms（架构 §8.1，测试内计时断言）
...
# ============== 用户 / 客户端 fixtures（范式同 test_etf_monitor_api.py）==============


@pytest_asyncio.fixture
async def normal_user(test_session):
    user = User(
        email="normal_market_metrics@example.com",
        password_hash="hash",
        role="user",
        is_active=True,
    )
...
async def _seed_metrics(test_session, metric_dates: list[date]):
    """种市场量价日汇总行；数值随日期递增，便于断言 latest 取值正确。"""
    for i, d in enumerate(metric_dates):
        test_session.add(
            MarketDailyMetric(
                trade_date=d,
                volume_shares=Decimal("1000000") * (i + 1),
                amount_yuan=Decimal("2000000000") * (i + 1),
                average_price=Decimal("10.5") + Decimal(i),
                expected_stock_count=5000 + i,
                daily_quote_count=4900 + i,
                suspended_stock_count=10 + i,
                final_stock_count=4910 + i,
            )
        )
    await test_session.commit()
...
async def seed_full_5(test_session):
    ...
    await _seed_metrics(test_session, days)
    return days


@pytest_asyncio.fixture
async def seed_gap_5x3(test_session):
    """5 个开市日，仅前 3 日有指标数据（后 2 日缺口）。"""
    days = _open_days(5)
    await _seed_calendar(test_session, days)
    await _seed_metrics(test_session, days[:3])
    return days
...
async def seed_full_3(test_session):
    """3 个开市日，全部有指标数据（range 大于已有开市日数场景）。"""
    days = _open_days(3)
    await _seed_calendar(test_session, days)
    await _seed_metrics(test_session, days)
    return days
...
# ============== 契约测试（camelCase / Decimal→float / ISO 日期）==============
...
class TestContract:
    async def test_full_data_contract(self, auth_client, seed_full_5):
        ...
        days = seed_full_5
        resp = await auth_client.get("/api/v1/market-metrics/trend")
        ...
        for p in data["points"]:
            ...
            assert isinstance(p["volumeShares"], float)
            assert isinstance(p["amountYuan"], float)
            assert isinstance(p["averagePrice"], float)
            assert isinstance(p["finalStockCount"], int)
            assert isinstance(p["suspendedStockCount"], int)
            date.fromisoformat(p["tradeDate"])  # ISO 格式合法

        # 升序 + 值与种子一致（首日 i=0 → volume=1e6）
        assert [p["tradeDate"] for p in data["points"]] == [
            d.isoformat() for d in days
        ]
        assert data["points"][0]["volumeShares"] == 1000000.0
        assert data["points"][0]["averagePrice"] == 10.5

        # latest = 最后一个有值点（即最后一日），结构与 point 一致
        assert data["latest"]["tradeDate"] == days[-1].isoformat()
        assert set(data["latest"].keys()) == POINT_KEYS
        assert data["latest"]["volumeShares"] == 5000000.0
        assert data["latest"]["finalStockCount"] == 4914
        assert data["latest"]["suspendedStockCount"] == 14
...
class TestGapContract:
    async def test_gap_points_are_null(self, auth_client, seed_gap_5x3):
        """5 日轴 + 3 日数据：5 点、缺失 2 点五项指标全 null、latest 为最后有值日"""
        days = seed_gap_5x3
        resp = await auth_client.get("/api/v1/market-metrics/trend")
        assert resp.status_code == 200
        data = resp.json()["data"]
        ...
        for p in missing:
            # 缺失日五项指标全 null —— 不补 0 / 前值（AC-06）
            assert p["volumeShares"] is None
            assert p["amountYuan"] is None
            assert p["averagePrice"] is None
            assert p["finalStockCount"] is None
            assert p["suspendedStockCount"] is None
            # 但日期本身不缺失（缺口轴锚点仍在）
            assert p["tradeDate"] is not None

        # latest 取最近"有结果"日 = 第 3 日（不是轴上最后一日，也不是今天）
        assert data["latest"]["tradeDate"] == days[2].isoformat()
        assert data["latest"]["volumeShares"] == 3000000.0
        assert data["latest"]["tradeDate"] != days[-1].isoformat()

    async def test_all_metrics_missing(self, auth_client, seed_calendar_only_4):
        """日历有开市日但指标全空：points 全 null、latest=null、hasMissingDates=true"""
        resp = await auth_client.get("/api/v1/market-metrics/trend")
        assert resp.status_code == 200
        data = resp.json()["data"]
        ...
        assert data["hasMissingDates"] is True
        assert all(p["volumeShares"] is None for p in data["points"])
...
class TestEmptyAndRange:
    ...
    async def test_range_30_90_250_cropping(self, auth_client, seed_full_100):
        """AC-05：100 个开市日全有数据 → range=30/90 返回 30/90 点，250 返回全部 100 点"""
        days = seed_full_100
        for rng, expected in ((30, 30), (90, 90), (250, 100)):
            resp = await auth_client.get(
                "/api/v1/market-metrics/trend", params={"range": rng}
            )
    ...
    async def test_invalid_range_rejected_422(self, auth_client, seed_full_5):
        ...
        resp = await auth_client.get(
            "/api/v1/market-metrics/trend", params={"range": 50}
        )
...
class TestAuth:
    async def test_requires_authentication(self, client):
        """未登录（无 token）→ 401"""
        resp = await client.get("/api/v1/market-metrics/trend")
...
# ============== 零 Provider 调用（架构 §4.2 模块 4、ADR-6）==============


class TestZeroProviderCalls:
    async def test_get_path_never_touches_providers(self, auth_client, seed_gap_5x3):
        """GET 路径零 Provider 调用：DataSourceFactory / TradingCalendar /
        TushareDataSource 全部 mock，断言从未被调用/实例化"""
        with (
            patch(
            ...
            resp = await auth_client.get(
                "/api/v1/market-metrics/trend", params={"range": 30}
            )
...
class TestPerformance:
    async def test_250_day_query_under_500ms(self, auth_client, seed_full_250):
        ...
        start = perf_counter()
        resp = await auth_client.get(
            "/api/v1/market-metrics/trend", params={"range": 250}
        )
